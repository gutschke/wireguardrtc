...
| 28 | `28_help_smoke.py`         | none        | none (just invokes daemon)     | Smoke test: `python3 wireguardrtc --help` must exit 0 AND produce non-empty stdout.  Catches the v0.2.0 corruption shape where the module-bottom `main` (no parens) made the daemon exit 0 silently.  Needs daemon's venv with websockets + pynacl (`$REPO/venv/` or `$PY` override). |
| 29 | `29_ast_lint.py`           | none        | none                           | AST-lint for missing-parens corruption: scans every Python executable in `daemon/` for (a) statement-level bare `Name`/`Attribute` expressions, (b) method-shaped attributes used in boolean contexts (`if not t.done`), (c) `name = obj.method` assignments where the attribute is a known method.  Triggers on the exact shape that bit v0.2.0; fast (<1 s); should run in CI before every daemon release. |
